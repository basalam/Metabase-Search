...
class TestDict(test.TestCase):
    ...
    def testTagged(self):
        ...
        for _ in range(4):
            ...
            expected = "cache:entry:hermes.test:Fixture:tagged:" + self._arghash(
                "alpha", b="beta"
            )
    ...
    def testFunction(self):
        ...
        for _ in range(4):
            ...
            self.assertEqual(
                {
                    "cache:entry:hermes.test.dict:foo:"
                    + self._arghash("alpha", "beta"): "ateb+ahpla"
                },
    ...
    def testCleanTagged(self):
        ...
        self.assertEqual(
            "ateb+ahpla",
            cache.pop(
                "cache:entry:hermes.test:Fixture:simple:"
                + self._arghash("alpha", "beta")
            ),
        )
        self.assertEqual(16, len(cache.pop("cache:tag:rock")))
        self.assertEqual(16, len(cache.pop("cache:tag:tree")))

        expectedKey = "cache:entry:hermes.test:Fixture:tagged:" + self._arghash(
            "gamma", "delta"
        )
        ...
        self.assertTrue(
            expectedKey in (":".join(k.split(":")[:-1]) for k in cache.keys())
        )
    ...
    def testNested(self):
        ...
        self.assertEqual(
            {
                "cache:entry:hermes.test:Fixture:nested:"
                + self._arghash("alpha", "beta"): "beta+alpha",
                "cache:entry:hermes.test:Fixture:simple:"
                + self._arghash("beta", "alpha"): "ahpla+ateb",
            },
            self.testee.backend.dump(),
        )
    ...
    def testConcurrent(self):
        ...
        self.assertEqual(
            "mk:alpha:beta", ":".join(tuple(cache.keys())[0].split(":")[:-1])
        )
        self.assertEqual("apabt", tuple(cache.values())[0])

        del log[:]
        self.testee.clean()
        self.testee.backend.lock = lambda k: lib.hermes.backend.AbstractLock(
            k
        )  # now see a dogpile
...
class TestDictCustomMangler(TestDict):
    ...
    def testSimple(self):
        ...
        for _ in range(4):
            self.assertEqual("ateb+ahpla", self.fixture.simple("alpha", "beta"))
            self.assertEqual(1, self.fixture.calls)
            key = "hermes:entry:hermes.test:Fixture:simple:" + str(
                self._arghash("alpha", "beta")
            )
            self.assertEqual({key: "ateb+ahpla"}, self.testee.backend.dump())
        ...
        for _ in range(4):
            self.assertEqual(
                expected, self.fixture.simple({"alpha": ["beta"]}, [{"gamma": "delta"}])
            )
            self.assertEqual(2, self.fixture.calls)
    ...
    def testTagged(self):
        ...
        for _ in range(4):
            ...
            self.assertFalse(
                cache.get("hermes:tag:tree") == cache.get("hermes:tag:rock")
            )
            self.assertTrue(int(cache.pop("hermes:tag:rock")) != 0)
            self.assertTrue(int(cache.pop("hermes:tag:tree")) != 0)

            expected = "hermes:entry:hermes.test:Fixture:tagged:" + str(
                self._arghash("alpha", "beta")
            )
            self.assertEqual(expected, ":".join(tuple(cache.keys())[0].split(":")[:-1]))
            self.assertEqual("ae-hl", tuple(cache.values())[0])
    ...
    def testKey(self):
        ...
        for _ in range(4):
            ...
            cache = self.testee.backend.dump()
            self.assertTrue(
                cache.get("hermes:tag:ash") != cache.get("hermes:tag:stone")
            )
            self.assertTrue(int(cache.pop("hermes:tag:ash")) != 0)
            self.assertTrue(int(cache.pop("hermes:tag:stone")) != 0)

            self.assertEqual(
                "mykey:alpha:beta", ":".join(tuple(cache.keys())[0].split(":")[:-1])
            )
            self.assertEqual("apabt", tuple(cache.values())[0])
    ...
    def testCleanTagged(self):
        ...
        self.assertEqual(
            "ateb+ahpla",
            cache.pop(
                "hermes:entry:hermes.test:Fixture:simple:"
                + str(self._arghash("alpha", "beta"))
            ),
        )
...
class TestDictCustomCached(test.TestCase):
    def setUp(self):
        self.testee = lib.hermes.Hermes(
            BackendThatIsDown, cachedClass=CustomCached, ttl=360
        )
        self.fixture = test.createFixture(self.testee)
